...
//            std::cout << "gauss: unique solution found" << std::endl;
    ...
    cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs)
    {
        ...
        const int n_matches = points_lhs.size(); // сколько матчей есть

        // https://en.wikipedia.org/wiki/Random_sample_consensus#Parameters
        const int n_trials = 100; // сколько попыток

        const int n_samples = 4; // сколько точек в одном семпле
        // по ощущениям логично из каких-то своих соображений взять количество точек в семпле и по формуле вывести нужное количство попыток для
        // правильной вероятности, но может сделаем это потом но пока вообще 4 точки у нас же метод Гауса
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            // теперь у нас есть рандомные точки
...
{
    double pt_homogeneous[3][1] = {{pt.x}, {pt.y}, {1.0}};
    cv::Mat transformed = T * cv::Mat(3, 1, CV_64F, pt_homogeneous);
    return cv::Point2d(
