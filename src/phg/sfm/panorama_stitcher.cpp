                            ...
                            std::function<cv::Mat(const cv::Mat &, const cv::Mat &)> &homography_builder)
{
...

    {
        ...
        int root_n = std::distance(parent.begin(), root_it);
        Hs[root_n] = cv::Mat::eye(3, 3, CV_64F);
        std::queue<int> q;
        q.push(root_n);
        while(!q.empty())
        {
            int root_now = q.front();
            q.pop();
            for(int i = 0; i < parent.size(); ++i)
            {
                if(parent.at(i) == root_now)
                {
                    q.push(i);
                    Hs[i] = Hs[root_now] * homography_builder(imgs[i], imgs[root_now]);
                }
            }
        }
    }
