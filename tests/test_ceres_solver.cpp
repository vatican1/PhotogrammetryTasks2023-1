...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Создаем функтор
    CostFunctor1 *f = new CostFunctor1(); // 10 - x
    ...
    // Параметры и якобианы - это указатели-т.е. список (столько сколько блоков невязки, в нашем случае - 1 блок) из указателей-т.е. списков
    // (каждый такой длинны, сколько параметров в этом отдельном блоке, у нас в единственном блоке единственный параметр - x)
    ...
    // потому что f(x) = 10 - x => f'(x) == -1 везде
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Проблема в том, что для задания прямой достаточно 2 коэффициентов, но тогда проблемы с вертикальными, поэтому? мы используем 3 переменные, надо превратить в 2, отнормируем)
        // Вообще по хорошему надо проверить, что мы не делим на ноль, но поскольку тут не раельные даные, и необходимо пройти 1 тест, скажем, что 3 коэффициент не 0
